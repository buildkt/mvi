...
class TimeTravelMiddlewareTest {
    @Test
    fun `initialize should add initial state when history is empty`() =
        runTest {
            val middleware =
    ...
    @Test
    fun `initialize should not add duplicate initial state`() =
        runTest {
            val initialState =
                StateSnapshot(
    ...
    @Test
    fun `onStateReduced should add state to history`() =
        runTest {
            val middleware =
                TimeTravelMiddleware<TestState, TestIntent>(
                    enable = true,
    ...
    @Test
    fun `onStateReduced should not record when disabled`() =
        runTest {
            val middleware =
                TimeTravelMiddleware<TestState, TestIntent>(
                    enable = false,
    ...
    @Test
    fun `onStateReduced should truncate history when exceeding maxHistorySize`() =
        runTest {
            val maxSize = 5
            val middleware =
                TimeTravelMiddleware<TestState, TestIntent>(
    ...
    @Test
    fun `restoreStateAt should restore state and update index`() =
        runTest {
            val middleware =
                TimeTravelMiddleware<TestState, TestIntent>(
                    enable = true,
            ...
            val success =
                middleware.restoreStateAt(1) { state ->
                    restoredState = state
                }
    ...
    @Test
    fun `loadHistory should reject invalid history`() =
        runTest {
            val middleware =
                TimeTravelMiddleware<TestState, TestIntent>(
                    enable = true,
