...
class StateHistoryStorageTest {
    ...
    @Test
    fun `saveHistory should store history`() = runTest {
        val storage = InMemoryStateHistoryStorage<TestState, TestIntent>()

        val history = listOf<StateSnapshot<TestState, TestIntent>>(
    ...
    @Test
    fun `loadHistory should return empty list when no history exists`() = runTest {
        val storage = InMemoryStateHistoryStorage<TestState, TestIntent>()
    ...
    @Test
    fun `saveHistory should create a copy to prevent external modifications`() = runTest {
        val storage = InMemoryStateHistoryStorage<TestState, TestIntent>()

        val originalHistory = mutableListOf<StateSnapshot<TestState, TestIntent>>(
            StateSnapshot(TestState(value = 0), null, index = 0),
        )
    ...
    @Test
    fun `loadHistory should return a copy to prevent external modifications`() = runTest {
        val storage = InMemoryStateHistoryStorage<TestState, TestIntent>()
    ...
    @Test
    fun `clearHistory should remove all stored history`() = runTest {
        val storage = InMemoryStateHistoryStorage<TestState, TestIntent>()
    ...
    @Test
    fun `saveHistory should overwrite previous history`() = runTest {
        val storage = InMemoryStateHistoryStorage<TestState, TestIntent>()

        val firstHistory = listOf<StateSnapshot<TestState, TestIntent>>(
            StateSnapshot(TestState(value = 0), null, index = 0),
        )

        val secondHistory = listOf<StateSnapshot<TestState, TestIntent>>(
    ...
    @Test
    fun `concurrent save and load should be thread-safe`() = runTest {
        val storage = InMemoryStateHistoryStorage<TestState, TestIntent>()
    ...
    @Test
    fun `saveHistory should handle empty list`() = runTest {
        val storage = InMemoryStateHistoryStorage<TestState, TestIntent>()
    ...
    @Test
    fun `clearHistory should be idempotent`() = runTest {
        val storage = InMemoryStateHistoryStorage<TestState, TestIntent>()
    ...
    @Test
    fun `storage should handle large history lists`() = runTest {
        val storage = InMemoryStateHistoryStorage<TestState, TestIntent>()

        val largeHistory = (0..1000).map { index ->
